...
- **세션 종료 전**: 의미 있는 작업을 했으면 위키 `Projects/__PROJECT_KEY__/log.md` 최신 날짜 섹션에 `- **제목**: 내용` 형식으로 기록하고 위키를 커밋·푸시한다. 제목은 한 줄로 압축한다 — 최신 섹션 제목이 다음 세션의 매 턴에 주입된다. Stop 훅이 기록 누락(코드 변경 + 오늘 기록 없음)이나 미저장 변경을 감지하면 경고한다.
...
- **머신별 경로**: 위키 위치의 단일 출처는 `.claude/settings.local.json`(git 미커밋)의 `env.WIKI_ROOT`. 커밋되는 파일에는 경로가 없다. 이 파일이 없으면 훅이 위키를 못 찾고 세션 시작 시 경고만 뜬다.
