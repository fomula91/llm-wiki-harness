...
문서 하나를 읽을 때도 통째로 읽지 마라 — `grep -n`으로 위치를 잡고 그 주변 블록만 읽는다.
읽은 내용은 그 세션의 매 턴 다시 전송된다.
...
- `log.md`는 최신 날짜가 위. 항목은 `- **제목**: 내용` 형식 — 코드 repo의 SessionStart 훅이 이 형식을 파싱한다. 제목은 한 줄로 압축한다 — 최신 섹션 제목 최대 10건이 코드 repo 세션의 매 턴에 주입된다.
