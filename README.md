...
설치하면 커맨드 두 개가 생긴다 (충돌 시 네임스페이스 형태 `/llm-wiki-harness:<name>`).

- `/llm-wiki-init` — 현재 저장소에 하네스 설치
- `/llm-wiki-migrate` — 구버전(훅 사본) 프로젝트를 현행으로 넘김 → [MIGRATION.md](MIGRATION.md)

### 이미 쓰고 있다면 (업그레이드)

**0.5.0부터 훅은 플러그인 안에서 실행된다.** 프로젝트로 복사되지 않으므로, 앞으로는 **플러그인만 업데이트하면 모든 프로젝트가 즉시 최신 훅을 쓴다.** 프로젝트별 재실행은 더 이상 필요 없다.

- **0.5.4** — 0.5.3 + CI 수정(테스트가 lib 전역을 직접 대입해 SC2034). 동작 동일.
- **0.5.3** — external 마이그레이션이 `settings.local.json`에 `WIKI_ROOT`를 심는다. 구버전 훅은 경로를 자기 안에 하드코딩해 뒀어서, 이걸 심지 않으면 마이그레이션이 하네스를 조용히 죽였다.
...
- **0.2.0** — 훅이 인라인 셸에서 별도 `.sh` 파일로 바뀌었다.

#### 0.5.0 마이그레이션

플러그인 안에서는 `/llm-wiki-migrate`, 터미널에서는:
...
> **절반만 하면 훅이 하나도 돌지 않는 `off` 상태가 된다.** 그런데 세션은 멀쩡히 진행되므로 에러 하나 안 난다 — 몇 주 뒤 "요즘 위키가 안 쌓이네"로 알아채게 된다. 그래서 `migrate.sh`는 절차를 안내하는 대신 끝까지 수행하고 결과 상태를 검증해 보고한다. 손으로 한다면 반드시 `./migrate.sh --check`로 `current`를 확인하고 끝낼 것.

전환 중 **훅이 두 벌 도는 구간은 없다.** `.claude/hooks/llm-wiki.conf.sh`가 남아 있는 동안 플러그인 훅은 스스로 물러선다. 기억이 두 번 주입되고 Stop이 이중으로 막는 것보다 잠깐 꺼지는 편이 낫다는 판단이고, `off` 상태는 그 대가다.

어느 경우든 기존 `settings.json`을 덮어쓰지 않는다 — `migrate.sh`는 `.bak`을 남기고, llm-wiki와 무관한 훅·설정은 보존된다.

> **플러그인 없이는 훅이 돌지 않는다.** clone해서 `install.sh`만 실행하면 위키 스켈레톤·`CLAUDE.md`·설정은 생기지만 SessionStart·Stop 훅은 붙지 않는다 — 훅은 플러그인이 `hooks/hooks.json`으로 제공하기 때문이다. 굳이 플러그인 없이 쓰려면 `settings.json`에 clone 경로의 훅을 직접 등록해야 하고, 그 순간 다시 "사본이 낡는" 문제로 돌아간다.
...
| 어울리는 경우 | 단일 repo로 완결, 협업자와 위키 공유(훅은 플러그인 설치자에게만) | 이미 중앙 vault 운영, 위키를 코드 repo에 노출하기 싫을 때 |
| install.sh | `./install.sh --in-repo <key> <repo>` | `./install.sh <key> <repo> <wiki-root>` |

## 하네스가 하는 일 (두 축)

**코드 repo 쪽** (`hooks/` + `project-side/`)
- `hooks/hooks.json` + `hooks/{lib,session-start,stop}.sh` — 훅 본체. **플러그인 안에서 그대로 실행된다**(`${CLAUDE_PLUGIN_ROOT}`). 프로젝트로 복사하지 않으므로 사본이 낡을 일이 없다. 프로젝트에 남는 것은 `.claude/llm-wiki.conf.sh`(키·모드·주입 상한) 하나뿐이고, **그 파일의 존재가 "여기 하네스가 설치됨"의 유일한 표식**이다 — 없으면 훅은 즉시 조용히 빠진다.
- 훅 2개:
...
- 이 플러그인이 설치된 머신 (훅이 플러그인 안에서 실행되므로, 머신마다 한 번 설치해야 한다)
...
- 훅은 JSON 안에 인라인된 한 줄 셸이 아니라 **별도 `.sh` 파일**로 배포한다. 읽고·디버깅하고·테스트할 수 있어야 하네스의 강제력을 증명할 수 있다.
- 훅을 프로젝트로 **복사하지 않는다.** 복사는 업데이트 문제를 만든다 — 사본은 만든 순간부터 낡기 시작하고, 갱신하려면 프로젝트마다 무언가를 실행해야 한다. 플러그인 안에서 실행하면 그 문제가 생기지 않는다. 부수 효과로 코드도 줄었다: 훅 복사, `settings.json`의 훅 병합, 사본 갱신 안내가 전부 사라졌다. 테스트가 배포본과 같은 파일을 돌린다는 성질은 더 강해졌다 — 이제 파일이 하나뿐이라 드리프트할 대상이 없다.
- 그 대가로 **플러그인을 설치하지 않은 사람에게는 훅이 돌지 않는다.** repo 내장 모드에서 위키 파일은 협업자와 공유되지만 기록 강제력은 걸리지 않는다. 위키를 코드와 함께 커밋한다는 이 모드의 이점은 그대로다.
- 마이그레이션 중 겹치면 플러그인 훅이 **물러선다**(구버전 사본 감지 시 무음). 두 벌이 함께 도는 것은 조용한 오작동이라 알아채기 어렵지만, 잠깐 꺼지는 것은 눈에 띄고 되돌리기도 쉽다. 강제력 하네스에서 "두 번 막기"는 "안 막기"보다 나쁘다.
- 플러그인 훅은 사용자의 **모든** 프로젝트에서 실행된다. 그래서 `conf.sh` 부재를 첫 관문으로 두고 무관한 프로젝트에서는 흔적조차 남기지 않는다 — 그 프로젝트에 우연히 `llm-wiki/`가 있어도 마찬가지다. `tests/`가 두 훅 모두에 대해 이 무음을 검사한다.
